...
export function Header() {
  ...
  const links = [['Runtime','/runtime'],['Architecture','/architecture'],['Experiments','/experiments'],['Research','/research'],['Benchmarks','/benchmarks'],['Articles','/articles'],['Docs','/documentation']]
  return <header className="relative z-20 border-b hairline">
    <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-5 lg:px-8">
      ...
      <div className="flex items-center gap-5"><RuntimeStatus /><button type="button" className="lg:hidden" onClick={() => setOpen(!open)} aria-expanded={open} aria-controls="mobile-navigation" aria-label={open ? 'Close navigation' : 'Open navigation'}>{open ? <X className="size-5" /> : <Menu className="size-5" />}</button></div>
    </div>
    {open && <nav id="mobile-navigation" aria-label="Mobile navigation" className="border-t hairline px-5 py-4 lg:hidden">{links.map(([label, href]) => <Link onClick={() => setOpen(false)} key={href} href={href} className="block py-3 mono text-xs text-muted-foreground">{label}</Link>)}</nav>}
  </header>
}
...
export function ArrowLink({ href, children }: { href: string; children: string }) { const dashboard = href === '/runtime'; const className = "group inline-flex items-center gap-2 border-b border-accent pb-2 mono text-[11px] tracking-[.08em] text-foreground"; const content = <>{children}<ArrowRight className="size-3 transition-transform group-hover:translate-x-1" /></>; return dashboard ? <a href="https://astra-os-mu.vercel.app/" target="_blank" rel="noopener noreferrer" className={className}>{content}</a> : <Link href={href} className={className}>{content}</Link> }
